...
unsigned _stackSize) : /*  stack size  */
		name(_p_name), semSend("TaskSem Send", 0), semReceive("TaskSem recv", 0) {
	StackType_t xStack[_stackSize];
	this->name = _p_name;
	p_data = NULL;
	...
	taskID = xTaskCreateStatic((TaskFunction_t) _taskCode,
			(const char*) this->name, (uint16_t) _stackSize, NULL,
			(UBaseType_t) aux_priority, xStack, &xTaskBuffer);
	if (taskID && xStack != NULL) {
...
TEDROOMPriority _priority, /*  task priority   */
unsigned _stackSize) : /*  stack size  */
		name("task"), semSend((unsigned int) 0), semReceive((unsigned int) 0)
...
void Pr_DelayAt(const Pr_Time &_time) {
	...
	if (tickCountFinal > tickCountNow) {
		...
		printf(
				"Pr_DelayAt(): El tiempo de finalizacion [%lu] no puede ser anterior al inicial [%lu].\n",
				(unsigned long) tickCountFinal, (unsigned long) tickCountNow);
	}
}
...
Pr_SemaphoreBin::Pr_SemaphoreBin(const char *p_name, uint32_t _value) :
	...
	xSemaphore = xSemaphoreCreateCountingStatic(1, _value, &xSemaphoreBuffer);
	if (xSemaphore != NULL) {
...
Pr_SemaphoreBin::Pr_SemaphoreBin(uint32_t _value) :
		Pr_Semaphore(_value) {
	name = "sem";
	xSemaphore = xSemaphoreCreateCountingStatic(1, _value, &xSemaphoreBuffer);
	if (xSemaphore != NULL) {
...
void Pr_SemaphoreBin::Signal() {
	if ( xSemaphoreGive( xSemaphore ) != pdTRUE) {
		printf(
				"xSemaphoreGive ERROR al devolver el semaforo: xSemaphoreGive(xSemaphore): %d\n",
				(int) xSemaphore);
	}
}
...
void Pr_SemaphoreBin::Wait() {
	if ( xSemaphoreTake(xSemaphore,
			( EDROOMClockTicksType ) portMAX_DELAY) != pdTRUE) {
		printf(
				"xSemaphoreTake ERROR al COGER el semaforo: xSemaphoreTake(xSemaphore, portMAX_DELAY): %d, %lu\n",
...
Pr_SemaphoreRec::Pr_SemaphoreRec(const char *p_name) :
		name(p_name) {
...
Pr_SemaphoreRec::Pr_SemaphoreRec(const char *p_name, int32_t prioceiling) :
		name(p_name)
...
void Pr_SemaphoreRec::UnLock() {
	if ( xSemaphoreGive( xSemaphore ) != pdTRUE) {
		printf(
				"ERROR al DEVOLVER el semaforo: xSemaphoreGive(xSemaphore): %d\n",
				(int) xSemaphore);
	}
}
...
void Pr_SemaphoreRec::Lock() {
	if ( xSemaphoreTake(xSemaphore,
			( EDROOMClockTicksType ) portMAX_DELAY) != pdTRUE) {
		printf(
				"ERROR al COGER el semaforo: xSemaphoreTake(xSemaphore, portMAX_DELAY): %d, %lu\n",
				(int) xSemaphore, (unsigned long) portMAX_DELAY);
	}
}
