...
void PUSService19::Exec19_1TC( CDTCDescriptor &TC, CDTMList &List){
  ...
  if(encontrado){
	  uint8_t service=GetUInt8(aux);
	  uint8_t subservice=GetUInt8(aux);
	  uint16_t length=GetUInt8(aux);
      EventAction[index].enabled=false;
      EventAction[index].RID=RID;
      EventAction[index].TC.dataFieldHeader.service=service;
      EventAction[index].TC.dataFieldHeader.subservice=subservice;
      EventAction[index].TC.packHeader.length=length;
      for(int i=0; i<length ; i++){
    	  EventAction[index].TC.appData[i]=GetUInt8(aux);
      }
