...
/* Task priorities. */
#define recv_task_PRIORITY (configMAX_PRIORITIES - 1)
#define send_task_PRIORITY (configMAX_PRIORITIES - 2)
/*******************************************************************************
 * Prototypes
 ******************************************************************************/
...
lpuart_rtos_config_t lpuart_config = { .baudrate = 115200, .parity =
...
static void recv_task(void *pvParameters) {
	...
	do {
		if (stream_ok()) {
				LPUART_RTOS_Send(&handle, (uint8_t*) recv_task_msg, strlen(recv_task_msg));
...
static void send_task(void *pvParameters) {
	...
	*/
	if (0 > LPUART_RTOS_Send(&handle, (uint8_t*) send_task_msg, strlen(send_task_msg))) {
			vTaskSuspend(NULL);
	}
	stream = get_stream();
	// Send bit stream.
