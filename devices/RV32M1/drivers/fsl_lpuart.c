 ...
 */
static void LPUART_WriteNonBlocking(LPUART_Type *base, const uint8_t *data,
		size_t length);
 ...
 */
static void LPUART_ReadNonBlocking(LPUART_Type *base, uint8_t *data,
		size_t length);
...
static size_t LPUART_TransferGetRxRingBufferLength(LPUART_Type *base,
	...
	if (handle->rxRingBufferTail > handle->rxRingBufferHead) {
		size = (size_t) (handle->rxRingBufferHead + handle->rxRingBufferSize
...
static bool LPUART_TransferIsRxRingBufferFull(LPUART_Type *base,
	...
	if (LPUART_TransferGetRxRingBufferLength(base, handle)
...
status_t LPUART_Init(LPUART_Type *base, const lpuart_config_t *config,
	...
	for (osrTemp = 4; osrTemp <= 32; osrTemp++) {
		...
		if (tempDiff
				> (config->baudRate_Bps
						- (srcClock_Hz / (osrTemp * (sbrTemp + 1))))) {
			tempDiff = config->baudRate_Bps
					- (srcClock_Hz / (osrTemp * (sbrTemp + 1)));
			sbrTemp++;
		}
	...
	temp |= (uint8_t) config->parityMode
			| LPUART_CTRL_IDLECFG(
					config->rxIdleConfig) | LPUART_CTRL_ILT(config->rxIdleType);
	 ...
	 */
	base->WATER = (((uint32_t) (config->rxFifoWatermark) << 16)
	...
	base->MODIR |= LPUART_MODIR_TXCTSC(
			config->txCtsConfig) | LPUART_MODIR_TXCTSSRC(config->txCtsSource);
...
void LPUART_Deinit(LPUART_Type *base) {
	...
	while (0
			!= ((base->WATER & LPUART_WATER_TXCOUNT_MASK)
...
status_t LPUART_SetBaudRate(LPUART_Type *base, uint32_t baudRate_Bps,
	...
	for (osrTemp = 4; osrTemp <= 32; osrTemp++) {
		...
		/* Select the better value between srb and (sbr + 1) */
		if (tempDiff
				> (baudRate_Bps - (srcClock_Hz / (osrTemp * (sbrTemp + 1))))) {
...
void LPUART_EnableInterrupts(LPUART_Type *base, uint32_t mask) {
	base->BAUD |= ((mask << 8)
			& (LPUART_BAUD_LBKDIE_MASK | LPUART_BAUD_RXEDGIE_MASK));
#if defined(FSL_FEATURE_LPUART_HAS_FIFO) && FSL_FEATURE_LPUART_HAS_FIFO
...
#if defined(FSL_FEATURE_LPUART_HAS_FIFO) && FSL_FEATURE_LPUART_HAS_FIFO
	base->FIFO =
			(base->FIFO & ~(LPUART_FIFO_TXOF_MASK | LPUART_FIFO_RXUF_MASK))
					& ~((mask << 8)
							& (LPUART_FIFO_TXOFE_MASK | LPUART_FIFO_RXUFE_MASK));
#endif
	mask &= 0xFFFFFF00U;
	base->CTRL &= ~mask;
}
...
uint32_t LPUART_GetEnabledInterrupts(LPUART_Type *base) {
	uint32_t temp;
	temp = (base->BAUD & (LPUART_BAUD_LBKDIE_MASK | LPUART_BAUD_RXEDGIE_MASK))
			>> 8;
#if defined(FSL_FEATURE_LPUART_HAS_FIFO) && FSL_FEATURE_LPUART_HAS_FIFO
	temp |= (base->FIFO & (LPUART_FIFO_TXOFE_MASK | LPUART_FIFO_RXUFE_MASK))
			>> 8;
#endif
...
#if defined(FSL_FEATURE_LPUART_HAS_FIFO) && FSL_FEATURE_LPUART_HAS_FIFO
	temp |= (base->FIFO
			& (LPUART_FIFO_TXEMPT_MASK | LPUART_FIFO_RXEMPT_MASK
					| LPUART_FIFO_TXOF_MASK | LPUART_FIFO_RXUF_MASK)) >> 16;
#endif
	return temp;
}
...
#endif
	temp &= (uint32_t) (~(LPUART_STAT_RXEDGIF_MASK | LPUART_STAT_IDLE_MASK
			| LPUART_STAT_OR_MASK | LPUART_STAT_NF_MASK |
			LPUART_STAT_FE_MASK | LPUART_STAT_PF_MASK));
	temp |= mask
			& (LPUART_STAT_RXEDGIF_MASK | LPUART_STAT_IDLE_MASK
					| LPUART_STAT_OR_MASK | LPUART_STAT_NF_MASK |
					LPUART_STAT_FE_MASK | LPUART_STAT_PF_MASK);
#if defined(FSL_FEATURE_LPUART_HAS_ADDRESS_MATCHING) && FSL_FEATURE_LPUART_HAS_ADDRESS_MATCHING
...
#if defined(FSL_FEATURE_LPUART_HAS_FIFO) && FSL_FEATURE_LPUART_HAS_FIFO
		while (0
				== ((base->WATER & LPUART_WATER_RXCOUNT_MASK)
...
status_t LPUART_TransferGetSendCount(LPUART_Type *base, lpuart_handle_t *handle,
		uint32_t *count) {
	assert(handle);
	assert(count);
		...
		if (handle->rxRingBuffer) {
			...
			if (bytesToCopy) {
				...
				for (i = 0U; i < bytesToCopy; i++) {
					xfer->data[bytesCurrentReceived++] =
							handle->rxRingBuffer[handle->rxRingBufferTail];

					/* Wrap to 0. Not use modulo (%) because it might be large and slow. */
					if (handle->rxRingBufferTail + 1U
			...
			if (0 == bytesToReceive) {
				if (handle->callback) {
					handle->callback(base, handle, kStatus_LPUART_RxIdle,
							handle->userData);
				}
			}
		}
...
void LPUART_TransferHandleIRQ(LPUART_Type *base, lpuart_handle_t *handle) {
	...
	if (LPUART_STAT_OR_MASK & base->STAT) {
		...
		/* Trigger callback. */
		if (handle->callback) {
			handle->callback(base, handle, kStatus_LPUART_RxHardwareOverrun,
					handle->userData);
		}
	}
	/* If IDLE flag is set and the IDLE interrupt is enabled. */
	if ((LPUART_STAT_IDLE_MASK & base->STAT)
			&& (LPUART_CTRL_ILIE_MASK & base->CTRL)) {
#if defined(FSL_FEATURE_LPUART_HAS_FIFO) && FSL_FEATURE_LPUART_HAS_FIFO
		count = ((uint8_t) ((base->WATER & LPUART_WATER_RXCOUNT_MASK)
				>> LPUART_WATER_RXCOUNT_SHIFT));
		...
		if ((handle->callback) && (handle->rxDataSize)) {
			handle->callback(base, handle, kStatus_LPUART_IdleLineDetected,
					handle->userData);
		}
	}
	/* Receive data register full */
	if ((LPUART_STAT_RDRF_MASK & base->STAT)
			&& (LPUART_CTRL_RIE_MASK & base->CTRL)) {
	...
	if ((base->STAT & LPUART_STAT_TDRE_MASK)
		...
		while ((count) && (handle->txDataSize)) {
			...
			if (!handle->txDataSize) {
				...
				/* Trigger callback. */
				if (handle->callback) {
					handle->callback(base, handle, kStatus_LPUART_TxIdle,
							handle->userData);
				}
			}
		}
...
void reset_handler() { //Resets counters and buffers
	irq_counter = 0;
	len = 0;
	header_ok = false;
	length_ok = false;
...
bool stream_ok() { //If appdata is ok, stream is complete
	return appdata_ok;
}

char* get_stream() { //Returns the full stream
	char str[sizeof(header) + sizeof(length) + sizeof(appData)] = "";
	strcat(str, header);
	strcat(str, length);
	strcat(str, appData);
	return strdup(str);
...
void LPUART0_DriverIRQHandler(void) {
	...
	aux = LPUART_ReadByte(LPUART0); //Receive input
	rxChars[irq_counter] = aux; //Save it in buffer
	LPUART_WriteByte(LPUART0, aux); //Show input
	irq_counter++;
